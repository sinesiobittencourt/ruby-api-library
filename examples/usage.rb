...
if fields && !fields.empty?
    fields.groups.each do |group|
        ...
        group.fields.each do |field|
            if field.type == KayakoClient::CustomField::TYPE_FILE
...
puts "Custom fields:"
fields = KayakoClient::CustomField.all
fields.sort{ |a,b| a.display_order <=> b.display_order }.each do |field|
    puts " #{field.custom_field_id}. #{field.title} [#{field.fieldname}]"
end

puts "Custom field options:"
options = KayakoClient::CustomFieldOption.all(1)
options.sort{ |a,b| a.display_order <=> b.display_order }.each do |option|
    puts " #{option.custom_field_option_id}. #{option.option_value}"
end
